...
var selectstroke='0',parce="''",bouton_distance=0,cumul=0,distance=0,surface=0,polygon=0,polyg='',indice='',count=0,image='',toMoveXold=0;
...
function create_use(id,fixe,x,y,coul,size,su,lie,elem,symbo,rota,contenu)
{
	...
	if(rota!="")
	{
	new_node.setAttributeNS(null,"transform","translate("+x+","+y+") rotate("+rota+")");
	}
	else
	{
	//new_node.setAttributeNS(null,"transform","matrix("+x+","+y+") rotate(180)");	
	new_node.setAttributeNS(null,"transform","matrix(1,0,0,-1,"+(x-taille/2)+","+(taille/2+parseFloat(y))+")");
	}
	
	new_node.setAttributeNS(null,"width",taille);
		new_node.setAttributeNS(null,"height",taille);
		//new_node.setAttributeNS(null,"x",0-taille/2);
		//new_node.setAttributeNS(null,"y",0-taille/2);
...
if(elem=='controlraster')
		...
		{
			fixe=data.getAttributeNS(null,"fixe");
			...
			size=data.getAttributeNS(null,"taille");
		for (var i=0; i<coorx.length; i++) 
		{
		su="switchColor(evt,'fill','"+surcoul+"','','"+ident[i]+"')";
		if(rotation!="")
		{
		create_use(ident[i],fixe,coorx[i],coory[i],coul[i],size,su,lie,elem,ref[i],rotation[i],contenu[i])
		}
		else
		{
		create_use(ident[i],fixe,coorx[i],coory[i],coul[i],size,su,lie,elem,ref[i],'',contenu[i])	
		}
		}
...
for(a=0;a<n;a++)
{
	...
	var axey="nok";
		if(zlayer[layer[b]].svg_visible=='true')
		{
			...
			if(zlayer[layer[b]].svg_zoomraster>zoomVal || ((nWidth*3300/larmap)>5221 && zlayer[layer[b]].svg_partiel=='1' && layer[b]!='000.postit'))
			{
...
function doPan(evt)
{
	...
	if (press==1 ) 
		...
		toMoveY1 = rect1UlYCorner+((newEvtY - vtY)/ratio)* theight / pluginPixHeight;
			...
			if((toMoveX > rectXcorner && (toMoveX + rectOvewidth) <allWidth) || (toMoveX < rectXcorner && (newEvtX - vtX) > 0) || ((toMoveX + rectOvewidth) >allWidth && (newEvtX - vtX) < 0))
			{
			...
			if(toMoveY > rectYcorner && (toMoveY + rectOveheight) < (allHeight) || (toMoveY < rectYcorner && (newEvtY - vtY) > 0) || ((toMoveY + rectOveheight) >allHeight && (newEvtY - vtY) < 0))
			{
...
{
coordonne(evt);
polyg = parseFloat(parseFloat(xini + xm))+ " " + parseFloat(parseFloat(yini- ym));
